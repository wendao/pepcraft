...
HIPModel::HIPModel(const char* filename)
{

  const int MaxSeqHP = 1000;

  char line[200]; //input line
  char pname[51];
  FILE* f;

  char s[101]; //multiple lines for one seq
  char* sHP;
  int flag = 0;
  ...
  for (n = 0; n < NumberOfMonomers; n++) {
    ...
    case 'H' : {seq[n] = hydrophobicH; break;}
    case 'I' : {seq[n] = hydrophobicI; break;}
  ...
  NumberOfHMonomers = 0;
  for (n = 0; n < NumberOfMonomers; n++)
    if (seq[n] == hydrophobicH) NumberOfHMonomers++;

  InternalHIContacts = 0;
  for (n = 0; n < NumberOfMonomers - 1; n++)
    if ((seq[n] == hydrophobicH) && (seq[n + 1] == hydrophobicI)) InternalHIContacts++;
    else if ((seq[n] == hydrophobicI) && (seq[n + 1] == hydrophobicH)) InternalHIContacts++;
...
HIPModel::~HIPModel()
{
  ...
  printf("Ciao HIPModel...\n");

}

void HIPModel::WriteState(int format, const char* filename)
{

  const char symbol[3] = {'H', 'I', 'P'};
...
void HIPModel::CheckModelIntegrity()
{
  ...
  for (n = 0; n < NumberOfMonomers - 1; n++) {
    ...
    if ((seq[n] == hydrophobicH) && (seq[n + 1] == hydrophobicI)) m++;
    else if ((seq[n] == hydrophobicI) && (seq[n + 1] == hydrophobicH)) m++;
  }

  if (InternalHIContacts != m) ErrorMsg(8);

  m = 0;
  for (n = 0; n < NumberOfMonomers; n++) {
    if (site[n] != GetSite(*coords[n])) ErrorMsg(8);
    if (OccupancyField -> CheckKey(site[n]) != n) ErrorMsg(8);
    if ((seq[n] != hydrophobicH) && (seq[n] != hydrophobicI) && (seq[n] != polar)) ErrorMsg(8);
    if (seq[n] == hydrophobicH) m++;
  }
...
void HIPModel::ErrorMsg(int message, const char* arg)
{

  fprintf(stderr, "\n");
  fprintf(stderr, "Error in 'HIPModel': ");
...
void HIPModel::GetObservablesHP()
{
  ...
  for (n = 0; n < NumberOfMonomers; n++) {
	...
	if ((nn != -1) && (seq[nn] != polar) && (seq[nn]!=seq[n])) HHContacts++;
  ...
  observable[0] = HHContacts - InternalHIContacts;

}

void HIPModel::GetObservablesPartHP(int nmin, int nmax, long int sign)
{
  ...
  for (; n <= nmax; n++) {
	...
	if ((nn != -1) && (seq[nn] != polar) && (seq[n] != seq[nn])) {
	...
	if ((nn != -1) && (seq[nn] != polar) && (seq[nn] != seq[n])) {
	  if ((nn < nmin) || (nn > nmax)) HHext++;
	  else HHint++;
	}
...
void HIPModel::GetObservablesISAW()
{
  observable[0] = 0;
}

void HIPModel::GetObservablesPartISAW(int nmin, int nmax, long int sign)
{
  observable[0] = 0;
}

void HIPModel::DoMCMove()
{
...
void HIPModel::InitPivotMatrices()
{
...
unsigned long int HIPModel::GetSite(Vector& v)
{
...
void HIPModel::BondRebridgingMove()
{
